...
inline std::vector<Categories> categories = {
    Categories{"Command History",
               {"@Command History", "$!!", "#Run the last command",
                "$chmod + x !$",
    ...
    Categories{"Creating Directories",
               {"@Creating Directories", "$mkdir foo", "# Create a directory",
                "$mkdir foo bar", "# Create multiple directories",
                "$mkdir -p|--parents foo/bar", "# Create nested directory",
                "$mkdir -p|--parents {foo,bar}/baz",
                "# Create multiple nested directories",
                "$mktemp -d|--directory", "# Create a temporary directory"}},
    Categories{"Moving Directories",
               {"@Moving Directories", "$cp -R|--recursive foo bar",
                "# Copy directory", "$mv foo bar", "# Move directory",
                "$rsync -z|--compress -v|--verbose /foo /bar",
                "# Copy directory, overwrites destination",
                "$rsync -a|--archive -z|--compress -v|--verbose /foo /bar",
    ...
    Categories{
        ...
        {"@Deleting Directories", "$rmdir foo", "# Delete empty directory",
         "$rm -r|--recursive foo", "# Delete directory including contents",
         "$rm -r|--recursive -f|--force foo",
         "# Delete directory including contents files and never prompt"}},
    Categories{"Creating Files",
               {"@Creating Files", "$touch foo.txt",
    ...
    Categories{
        "Standard Output",
        {"@Standard Output", "$echo foo > bar.txt",
         "# Overwrite file with content", "$echo foo >> bar.txt",
         "# Append to file with content", "$ls exists 1> stdout.txt",
         "# Redirect the standard output to a file",
         "$ls noexist 2> stderror.txt",
         "# Redirect the standard error output to a file", "$ls 2>&1 > out.txt",
         "# Redirect standard output and error to a file", "$ls > /dev/null",
         "# Discard standard output and error", "$read foo",
         "# Read from standard input and write to the variable foo"}},
    Categories{"Moving Files",
               {"@Moving Files", "$cp foo.txt bar.txt", "# Copy file",
                "$mv foo.txt bar.txt", "# Move file",
                "$rsync -z|--compress -v|--verbose /foo.txt /bar",
                "# Copy file quickly if not changed",
                "$rsync z|--compress -v|--verbose /foo.txt /bar.txt",
                "# Copy and rename file quickly if not changed"}},
    Categories{"Deleting Files",
               {"@Deleting Files", "$rm foo.txt", "# Delete file",
                "$rm -f|--force foo.txt",
                "# Delete file, ignore nonexistent files and never prompt"}},
    Categories{"Reading Files",
               {"$cat foo.txt", "# Print all contents", "$less foo.txt",
    ...
    Categories{
        ...
        {"@Replace in Files", "$sed 's/fox/bear/g' foo.txt",
         ...
         "# Replace fox (case insensitive) with bear in foo.txt and output",
         "$sed 's/red fox/blue bear/g' foo.txt",
         "# Replace red with blue and fox with bear in foo.txt and output",
    ...
    Categories{"Symbolic Links",
               {"@Symbolic Links", "$ln -s|--symbolic foo bar",
                "# Create a link 'bar' to the 'foo' folder",
                "$ln -s|--symbolic -f|--force foo bar",
    ...
    Categories{
        ...
        {"@Compressing Files",
         ...
         "$tar -c",
         "#Compresses (optionally) and combines  files into a single *.tar",
    ...
    Categories{"Decompressing Files",
               {"@Decompressing Files", "@zip", "$unzip", "$unzip foo.zip",
                "# Unzip foo.zip into current directory", "$gunzip",
                "$gunzip foo.gz",
                "# Unzip foo.gz into current directory and delete foo.gz",
                "$gunzip -k|--keep foo.gz",
                "# Unzip foo.gz into current directory", "@gzip", "$tar -x",
    ...
    Categories{
        "Disk Usage",
        {"@Disk Usage", "$df", "# List disks, size, used and available space",
         "$df -h|--human-readable",
         "# List disks, size, used and available space in a human readable",
         ...
         "$du -d|--max-depth",
         "# List subdirectories and file sizes within the max depth",
         "$du -d 0", "# List current directory size"}},
    Categories{
        "Memory Usage",
        {"@Memory Usage", "$free", "# Show memory usage", "$free -h|--human",
         "# Show human readable memory usage", "$free -h|--human --si",
         "# Show human readable memory usage in power of 1000",
         "$free -s|--seconds 5",
         "# Show memory usage and update continuously every five seconds"}},
    Categories{"Package Management",
               {"@Package Management", "$pacman -S|--sync package",
                "# Install package", "$pacman -Sy|--refresh",
                "# Update package database", "$pacman -R|--remove package",
                "# Remove package", "$pacman -U|--upgrade package",
                "# Upgrade package", "$pacman -Q|--query package",
                "# Query package", "$pacman -Qm|--query foreign package",
                "# Query foreign package", "$pacman -Qi|--queryinfo package",
                "# Query package information"}},
    Categories{"Shutdown and Reboot",
               {"@Shutdown and Reboot", "$shutdown", "# Shutdown in 1 minute",
                ...
                "$shutdown -c", "# Cancel a shutdown or reboot", "$reboot",
                "# Reboot now", "$reboot -f", "# Force a reboot"}},
    Categories{
    ...
    Categories{"Process Priority",
               {"@Process Priority", "$nice -n -20 foo",
                "# Change process priority by name", "$renice 20 PID",
                "# Change process priority by PID", "$ps -o ni PID",
                "# Return the process priority of PID"}},
    Categories{"Killing Processes",
               {"@Killing Processes", "CTRL+C",
                "# Kill a process running in the foreground", "$kill PID",
    ...
    Categories{"Time and Date",
               {"@Time and Date", "$date", "# Print the date and time",
    ...
    Categories{"Schedule Tasks",
               {"@Schedule Tasks",
                ...
                "# Load cron tab from a file",
                "$crontab -l > /path/crontab",
                "# Save cron tab to a file",
                "",
                "* * * * * foo",
                ...
                "$at now + 2 minutes",
                "# Create a task in Vim to execute in 2 minutes",
                "$at 12:34 PM next month",
                "# Create a task in Vim to execute at 12:34 PM next month",
                "$at tomorrow",
    ...
    Categories{
        ...
        {"@Https Requests",
         ...
         "# Follow redirects",
         "$curl -o|--remote-name foo.txt https://example.com",
         "# Output to a text file",
         "$curl -H|--header 'User-Agent: Foo' https://example.com",
         "# Add a HTTP header",
    ...
    Categories{"Dns Troubleshooting",
               {"@Dns Troubleshooting", "$host example.com",
                "# Show the IPv4 and IPv6 addresses", "$dig example.com",
                "# Show complete DNS information", "$cat /etc/resolv.conf",
                "# resolv.conf lists nameservers"}},
    Categories{"Hardware",
               {"@Hardware", "$lsusb", "# List USB devices", "$lspci",
                "# List PCI hardware", "$lshw", "# List all hardware"}},
    Categories{"Terminal Multiplexers",
               {"@Terminal Multiplexers",
                "#Start multiple terminal sessions. Active sessions persist",
                "tmux is more modern than screen.", "$tmux",
                "# Start a new session (CTRL-b + d to detach)", "$tmux ls",
                "# List all sessions", "$tmux attach -t 0",
                "# Reattach to a session", "$screen",
                "# Start a new session (CTRL-a + d to detach)", "$screen -ls",
                "# List all sessions", "$screen -R 31166",
    ...
    Categories{"Shell Profile",
               {"@Bash Profile", "#bash - .bashrc", "#zsh - .zshrc",
                "# Always run ls after cd", " function cd {",
                " builtin cd '$@' && ls", " }", "",
                "# Prompt user before overwriting any files",
                " alias cp='cp --interactive'", " alias mv='mv --interactive'",
